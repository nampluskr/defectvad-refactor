...
def assert_disjoint(split_dict):
    sets = {name: set(split_dict[name]) for name in ("train", "valid", "test") if name in split_dict}
    pairs = [("train", "valid"), ("train", "test"), ("valid", "test")]
    for name1, name2 in pairs:
        if name1 in sets and name2 in sets:
...
def generate_ratio_split(all_ids, ratio, seed, stratify_by=None):
    ...
    if stratify_by is None:
        rng.shuffle(ids)
        n = len(ids)
        n_train = int(n * ratio.get("train", 0.0))
        n_valid = int(n * ratio.get("valid", 0.0))
        split_dict = {
            "train": ids[:n_train],
